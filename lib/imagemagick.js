...
class ImageMagick {
  ...
  async identify(imagePath) {
    ...
    if (this.magickCmd === 'magick') {
      ...
      output = await this.executeCommand(args);
    } else {
    ...
    if (info.width === 0 || info.height === 0) {
      ...
      if (this.magickCmd === 'magick') {
        ...
        verboseOutput = await this.executeCommand(args);
      } else {
        try {
          const escapedPath = imagePath.replace(/"/g, '\\"');
  ...
  async watermark(inputPath, outputPath, options = {}) {
    ...
    if (type === 'image' && watermarkImage) {
      ...
      await this.executeCommand([...watermarkArgs, tempWatermark]);
    ...
    if (type === 'text' && text) {
      ...
      // 创建文字图片
      await this.executeCommand(textArgs);
      ...
      if (opacity < 1.0) {
        ...
        await this.executeCommand(opacityArgs);
      }
      
      // 旋转文字（如果设置了角度）
      if (angle !== 0) {
        const rotateArgs = [textImage, '-rotate', angle.toString(), textImage];
        await this.executeCommand(rotateArgs);
      }

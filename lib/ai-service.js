...
- ⚠️ 如果用户请求的操作类型不在上述支持列表中，必须返回错误信息，格式：{"error": "不支持的操作类型：xxx，请使用支持的操作类型"}
- ⚠️ 不要尝试猜测或使用未列出的操作类型，直接提示无法处理
...
- 如果遇到不支持的操作类型，必须返回错误信息：
{
  "error": "不支持的操作类型：xxx，请使用支持的操作类型。支持的操作类型包括：resize, crop, shapeCrop, rotate, convert, watermark, adjust, grayscale, blur, sepia, negate, sharpen 等"
}

⚠️ 重要：不要尝试猜测或使用未列出的操作类型，如果用户请求的操作不在支持列表中，必须返回error字段，不要返回operations数组。`;
      ...
      if (!result.operations || !Array.isArray(result.operations)) {
        throw new Error('AI返回的格式不正确，缺少operations数组或error字段');
      }
